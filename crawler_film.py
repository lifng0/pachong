...
pages1 = 5
base_film_url = 'https://movie.douban.com/top250?start='
for page1 in range(pages1):
    film_url = base_film_url+str(page1*25)
    resp1 = requests.get(film_url, headers=headers)
    film_bs = BeautifulSoup(resp1.text, 'html.parser')
    film_item = film_bs.find_all('div',{'class':'item'})
    for item in film_item:
        comment_lib = []
        film_name = item.find('span',{'class':'title'}).get_text(strip=True)
        film_id = item.find('a',href = True)['href'].split('/')[-2]
        ...
        with open(f'{film_name}.json', 'w', encoding='utf-8') as f:
